...
- **要件定義書**：[要件定義.md](要件定義.md)（`要件定義.md` = Requirements Definition。英語圏メンバーやツールでは requirements-definition.md 相当）に、プロジェクトの目的・背景・As-Is/To-Be・スコープ・ゴール・成功基準・成果物を定義している。
...
本プロジェクトにおける主な設計判断は以下のとおりである。詳細は [要件定義](要件定義.md) および各標準を参照。
...
|-------|------|------|---------------------------|
| **Phase 1** | 現在 | データ取り込み、集計、ダッシュボード可視化、JEDAI での紹介 | 目標期日：2025年3月末。詳細は [要件定義 3.6](docs/要件定義.md#36-マイルストーン) 参照 |
...
**Phase 1 マイルストーン**（[要件定義 3.6](docs/要件定義.md#36-マイルストーン)）
...
| 日本語 | 英語（相当） | 用途 |
|--------|--------------|------|
| `要件定義.md` | Requirements Definition（requirements-definition.md） | プロジェクトの要件・スコープ・ゴール・成果物 |
...
**成功基準と受入基準**（詳細は [要件定義 3.2](要件定義.md#32-成功基準)）
