...
- **要件定義書**：[docs/要件定義.md](docs/要件定義.md)（Requirements Definition）に、プロジェクトの目的・背景・As-Is/To-Be・スコープ・ゴール・成功基準・成果物を定義している。
...
詳細は [docs/要件定義.md](docs/要件定義.md) の「2. As-Is / To-Be」「3. ゴールと成功基準」「4. スコープ定義」を参照。
...
ビジネス背景・目的・スコープは下記 [プロジェクトサマリ](#プロジェクトサマリ) および [docs/要件定義.md](docs/要件定義.md) を参照。
...
- **コミュニティ規模**：数百〜数千メンバー程度を想定する（要件定義 1.5）
...
本プロジェクトにおける主な設計判断は以下のとおりである。詳細は [docs/要件定義.md](docs/要件定義.md) および各標準を参照。
...
| Phase | 状況 | 焦点 | マイルストーン（Phase 1） |
|-------|------|------|---------------------------|
| **Phase 1** | 現在 | データ取り込み、集計、ダッシュボード可視化、JEDAI での紹介 | 目標期日は [要件定義 3.6](docs/要件定義.md#36-マイルストーン) に準拠。進捗は **Azure DevOps を正**とする。詳細は [docs/backlog.md](docs/backlog.md) を参照。 |
...
**Phase 1 マイルストーン**（[要件定義 3.6](docs/要件定義.md#36-マイルストーン) に準拠。進捗の正は Azure DevOps。Phase 1 進捗の詳細は [docs/backlog.md](docs/backlog.md) を参照。）
...
| 日本語 | 英語（相当） | 用途 |
|--------|--------------|------|
| `docs/要件定義.md` | Requirements Definition | プロジェクトの要件・スコープ・ゴール・成果物 |
...
本セクションは [docs/要件定義.md](docs/要件定義.md) の要約である。スコープ・ゴール・成果物の正式な定義は要件定義を正とする。
...
**ゴール**：2025年3月末までに、Discord コミュニティの活動傾向を**曜日**・**時間帯**で説明できるダッシュボードを構築する。

**成功基準と受入基準**（詳細は [要件定義 3.2](docs/要件定義.md#32-成功基準)）
...
| **Gold** | ダッシュボード用集計（曜日、時間帯等）。4 本（activity_by_weekday_hour, activity_daily, user_activity, channel_activity）。全テーブルに guild_id / guild_name を持ちギルド別絞り込み・比較が可能。集計値カラムは _aggregated サフィックス | フルリフレッシュ時は `overwriteSchema` で上書き |
